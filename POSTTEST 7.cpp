...
// Header dipakai untuk mengolah file eksternal
#include <fstream>
fstream file;
...
int    checkInt	          ()                                                     ;
void   checkFile          (dataPengunjung struk[])                               ;
void   saveFile           (dataPengunjung struk[])                               ;
void   loadFile           (dataPengunjung struk[])                               ;
...
void   resetProgram		  (dataPengunjung struk[])                               ;
...
// Default
void   dataDefault        (dataPengunjung struk[])                               ;
void   dataReset          (dataPengunjung struk[])                               ;
...
int main(){
	...
	cout << " Menu Interaktif: \n" 	  	    << endl
		 ...
		 << "\t [S / 6] Data Nama"		    << endl
		 << "\t [C / 7] Reset Data"			<< endl
		 << "\t [X / 8] Hentikan Program"	<< endl;
	ccout("Abu-abu"," *Pastikan untuk menutup program menggunakan");
	ccout("Abu-abu","  opsi \"Hentikan Program\" agar data tersimpan.");
	...
	switch(responMenu){
		...
		case '7': case 'C': case 'c':
			resetProgram(kunjung);
			break;

		// Quit
	 	case '8': case 'X': case 'x':
	 		berjalan = false;
	 		if (jumlahKamar != 0){
	 		saveFile(kunjung);}
	 		break			;}
...
// Untuk Mengecek Ada Tidaknya File
void checkFile(dataPengunjung struk[]){
    file.open("Data hotel.csv", ios::in);
...
void loadFile(dataPengunjung struk[]){
    file.open("Data hotel.csv", ios::in);
    string num, wniString, room, observator;
    
    for(int i = 0; i < 50; i++){
        getline(file, num,                    ',');
		
        // untuk mengonversi string ke tipe data lain
        stringstream nomoring(num);
		nomoring >> struk[i].nomorKamar;
        
        getline(file, struk[i].nomorPengenal,   ',');
        getline(file, struk[i].nama,            ',');
        getline(file, struk[i].tipeKamar,       ',');
		getline(file, wniString,                ',');
		
		if (wniString != "0" || wniString != "1"){
        	struk[i].wni = 1;}
        else{
        	struk[i].wni = (wniString == "0")? 1 : 0;
		}
		
        getline(file, struk[i].tanggalCheckin,  ',');
        getline(file, struk[i].tanggalCheckout, ',');
        getline(file, struk[i].catatan, '\n');
        }
    
    getline(file, room,       ',');
    stringstream rooming(room);
    rooming >> jumlahKamar;
    
    getline(file, observator,       '\n');
    stringstream observating(observator);
	observating >> pengunjung;
...
void saveFile(dataPengunjung struk[]){
	file.open("Data hotel.csv", ios::out);
    
	for (int i = 0; i < 50; i++){
        file << struk[i].nomorKamar      << ','
			 << struk[i].nomorPengenal   << ','
			 << struk[i].nama            << ','
			 << struk[i].tipeKamar       << ','
			 << struk[i].wni             << ','
			 << struk[i].tanggalCheckin  << ','
			 << struk[i].tanggalCheckout << ','
		     << struk[i].catatan         << endl;}
...
void dataTampil(dataPengunjung struk[]){
	...
	if(jumlahKamar == 0){
		ccout("Biru",   "    Belum Ada Kamar di Hotel Ini.");
		ccout("Kuning", "         [Tekan apa saja.]"       );
		getch();
	}
}
...
void resetProgram(dataPengunjung struk[]){
	...
	ccout("Merah", " Ketik \"RE5ET P4BR1K\" untuk menghapus data keseluruhan");
	ccout("Merah", " Ketik \"RE5ET T0T4L\" untuk mengembalikan data ke default");
	
	cout << " Konfirmasi:";
	getline(cin, konfirmasi);
	fflush(stdin);
	
	if (konfirmasi == "RE5ET P4BR1K"){
		dataDefault(struk);
		saveFile(struk);
		ccout("Biru",  "\t Penghapusan Berhasil.");
		ccout("Kuning", "\t   [Tekan apa saja.]") ;}
		
	else if (konfirmasi == "RE5ET T0T4L"){
		dataReset(struk);
		jumlahKamar = 0;
		remove("Data hotel.csv");
		ccout("Biru",  "\t Penghapusan Berhasil.");
		ccout("Kuning", "\t   [Tekan apa saja.]") ;}
	
	else{
		ccout("Merah",  "\t Penghapusan Dibatalkan.");
		ccout("Kuning", "\t   [Tekan apa saja.]") ;
	}
	getch();
}
...
void editHotel(dataPengunjung struk[]){
	...
	switch(opsi){
		case '1':
			...
			if(jumlahKamar >= 49){
				ccout("Merah",  "\t\t Kapasitas Kamar Penuh.");
				ccout("Kuning", "\t\t   [Tekan apa saja.]"   );
				getch()                                       ;
				break                                         ;
			}
		...
		case '2':
			...
			if (index == 99){cout << endl;}
			else if(index == 50){editHotel(struk);}
...
void dataDefault(dataPengunjung struk[]){
	...
	struk[index].tipeKamar = "superior room"        ; 
	index++						   	   				;
